...
def main(df, mpath, revisions):
    ...
    for checkpoint in tqdm(revisions):
        ...
        for seed in range(1, 10):
            ...
            savepath = "data/processed/rawc/pythia/attention_rs/"
            if not os.path.exists(savepath): 
                os.mkdir(savepath)
            if "/" in mpath:
                filename = "rawc-attentions-rs_model-" + mpath.split("/")[1] + "-" + checkpoint +  "-" + seed_name +".csv"
            else:
                filename = "rawc-attentions-rs_model-" + mpath +  "-" + checkpoint + "-" + seed_name +".csv"
