...
pca_features <- df_ttest_r2_attns %>%
  select(Coefficient, t_statistic, is_sig, N, V) %>%
  scale()  # Standardize
...
df_ttest_r2_attns %>%
  mutate(label = ifelse(disambig_PC1 <= quantile(disambig_PC1, 0.025),
                        paste(Layer, Head, sep = "_"),
                        NA)) %>%
...
df_ttest_r2_attns %>%
  mutate(label = ifelse(disambig_PC1 <= quantile(disambig_PC1, 0.025) & is_sig == TRUE,
                        paste(Layer, Head, sep = "_"),
                        NA)) %>%
