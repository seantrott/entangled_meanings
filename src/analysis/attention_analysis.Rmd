...
summary(lmer(data = pythia_410m, 
             avg_entropy ~ log10(step_modded) + (1 | Head) + (1 | Layer)))



summary(lm(data = pythia_410m, 
             avg_entropy ~ log10(step_modded) * Layer))
...
# Manuscript figure (Fig.1c)
df_attn_by_head %>%
  group_by(mpath) %>%
  mutate(mean_attention_scaled = scale(mean_attention)) %>%
  ggplot(aes(x = Layer,
             y = Head,
             fill = mean_attention_scaled)) +
  geom_tile() +
  labs(x = "Layer",
       y = "Head",
       fill = "Mean Attn. (Z-scored)") +
  scale_fill_gradient2(low = "blue",
                       mid = "white",
                       high = "red",
                       # midpoint = 0, 
                       name = "Mean Attn. (Z-scored)") +
  theme_minimal() +
  theme(text = element_text(size = 15),
        strip.text.y = element_text(angle = 0), # Keep facet labels horizontal
        axis.text.x = element_text(angle = 0, hjust = 1),
...
summ_stats = df_most_attentive_heads %>%
...
summ_stats %>%
  ...
  geom_text_repel(aes(label=mpath), size=3) +
  labs(x = "#Heads Overall",
       y = "Proportion of Candidate Heads",
       color = "",
       shape = "") +
  theme_minimal() +
  # guides(color="none") +
...
# Manuscript Figure (Appendix FigS2)
df_attention_over_time %>%
  # filter(Layer == 3) %>%
  ggplot(aes(x = step_modded,
             y = mean_attention,
...
df_best_r2_14m = df_pythia_models_distance %>% 
  filter(mpath %in% c("EleutherAI/pythia-14m")) %>%
...
df_best_r2_410m = df_pythia_models_distance %>% 
...
## Merge Attention and R2 data

```{r}
df_attention_over_time_14m = df_pythia_models %>% 
  filter(mpath %in% c("EleutherAI/pythia-14m")) %>%
  group_by(mpath, revision, Layer, Head, step, n_params) %>%
  summarise(mean_attention = mean(Attention)) %>%
  mutate(step_modded = step + 1) %>%
...
df_merged_r2_attention_14m = df_attention_over_time_14m %>%
  inner_join(df_best_r2_14m)


scale_factor <- max(df_merged_r2_attention_14m$mean_attention, na.rm = TRUE) / 
                max(df_merged_r2_attention_14m$r2, na.rm = TRUE)

# Manuscript Figure (Fig. 2c)
df_merged_r2_attention_14m %>%
  filter(Layer == 3) %>%
  # filter(Head %in% c(1, 2)) %>%
  ggplot(aes(x = step_modded, color = factor(Head))) + 
  # Primary Y-axis: Attention
  geom_line(aes(y = mean_attention), size = 3, alpha = .7) +
...
# --- for Pythia-410M
df_attention_over_time_410m = df_pythia_models %>% 
  filter(mpath %in% c("EleutherAI/pythia-410m")) %>%
  group_by(mpath, revision, Layer, Head, step, n_params) %>%
  summarise(mean_attention = mean(Attention)) %>%
  mutate(step_modded = step + 1) %>%
...
df_merged_r2_attention_410m = df_attention_over_time_410m %>%
  inner_join(df_best_r2_410m)


df_merged_r2_attention_410m %>%
  ggplot(aes(x = step_modded, color = factor(Head))) +
  # Primary Y-axis: Attention
  geom_line(aes(y = mean_attention), size = 3, alpha = .7) +
  # Secondary Y-axis: R-squared
  ...
  facet_wrap(~Layer) +
  scale_color_manual(values = viridisLite::viridis(16, option = "mako", 
                                                   begin = 0.1, end = 0.9)) +
  theme(
...
SELECT_LAYERS_410M = c(1,2,4,5,6,10,13,22,24)

df_merged_r2_attention_410m %>%
  filter(Layer %in% SELECT_LAYERS_410M) %>%
  ggplot(aes(x = step_modded, color = factor(Head))) +
  # Primary Y-axis: Attention
  geom_line(aes(y = mean_attention), size = 3, alpha = .7) +
  geom_vline(xintercept = 1000, 
...
df_best_r2_layer = df_pythia_models_distance %>% 
  filter(step == 143000) %>%
  filter(Layer > 0) %>%
...
df_best_r2_layer = df_best_r2 %>%
  filter(step == 143000) %>%
  group_by(mpath) %>%
  slice_max(r2, n = 1, with_ties = FALSE) %>%
  dplyr::select(mpath, Layer) 
...
df_wide <- df_merged_r2_attention_actual %>%
  dplyr::select(revision, Layer, Head, step, mean_attention, r2) %>%
  group_by(revision) %>%
  slice_max(r2) %>%
...
t_test_results_subset = t_test_results %>%
  select(-t_test)

write.csv(t_test_results_subset, "../../data/processed/rawc/pythia/summaries/t_test_results.csv")
...
write.csv(regression_results_df, "../../data/processed/rawc/pythia/summaries/r2_results_410m.csv")
```
